...
// Product data with extensive JSON attributes
const products = [
  {
...
let catalogSQL = '-- Catalog Products (Generated with crypto.randomUUID())\n';
let pricingSQL = '-- Pricing Data (Generated with crypto.randomUUID())\n';
let fulfillmentSQL = '-- Inventory Data (Generated with crypto.randomUUID())\n';
...
products.forEach((product, index) => {
  ...
  // Catalog SQL
  catalogSQL += `INSERT INTO products (product_id, data, created_at, updated_at) VALUES (\n`;
  catalogSQL += `  '${productId}',\n`;
  catalogSQL += `  '${dataJson}',\n`;
  catalogSQL += `  '${now}',\n`;
  catalogSQL += `  '${now}'\n`;
  catalogSQL += `);\n\n`;
  ...
  // Pricing SQL
  pricingSQL += `INSERT INTO prices (product_id, price, currency, created_at, updated_at) VALUES (\n`;
  pricingSQL += `  '${productId}',\n`;
  pricingSQL += `  ${product.price},\n`;
  pricingSQL += `  'INR',\n`;
  pricingSQL += `  '${now}',\n`;
  pricingSQL += `  '${now}'\n`;
  pricingSQL += `);\n\n`;
  
  // Fulfillment SQL (distribute stock across warehouses)
  // For simplicity, assign to first warehouse (WH-MUM-001)
  // In production, you'd distribute across multiple warehouses
  const inventoryId = randomUUID();
  fulfillmentSQL += `INSERT INTO inventory (inventory_id, product_id, warehouse_id, quantity, reserved_quantity, created_at, updated_at) VALUES (\n`;
  fulfillmentSQL += `  '${inventoryId}',\n`;
  fulfillmentSQL += `  '${productId}',\n`;
  fulfillmentSQL += `  'WH-MUM-001',\n`;
  fulfillmentSQL += `  ${product.stock},\n`;
  fulfillmentSQL += `  0,\n`;
  fulfillmentSQL += `  '${now}',\n`;
  fulfillmentSQL += `  '${now}'\n`;
  fulfillmentSQL += `);\n\n`;
  
  console.log(`Generated UUID for ${product.name}: ${productId}`);
});

// Write SQL files
const dbSchemasDir = path.join(__dirname, '..', 'database-schemas');
fs.mkdirSync(dbSchemasDir, { recursive: true });

fs.writeFileSync(
  path.join(dbSchemasDir, 'sample-products-uuid.sql'),
  catalogSQL
);

fs.writeFileSync(
  path.join(dbSchemasDir, 'sample-prices-uuid.sql'),
  pricingSQL
);

fs.writeFileSync(
  path.join(dbSchemasDir, 'sample-inventory-uuid.sql'),
  fulfillmentSQL
);

console.log('\n✅ Generated SQL files:');
console.log('  - database-schemas/sample-products-uuid.sql');
console.log('  - database-schemas/sample-prices-uuid.sql');
console.log('  - database-schemas/sample-inventory-uuid.sql');
console.log('\n📝 Next steps:');
console.log('  1. Execute catalog SQL: wrangler d1 execute catalog-db --file=./database-schemas/sample-products-uuid.sql --config wrangler.catalogworker.toml');
console.log('  2. Execute pricing SQL: wrangler d1 execute pricing-db --file=./database-schemas/sample-prices-uuid.sql --config wrangler.pricingworker.toml');
console.log('  3. Execute inventory SQL: wrangler d1 execute fulfillment-db --file=./database-schemas/sample-inventory-uuid.sql --config wrangler.fulfillmentworker.toml');
